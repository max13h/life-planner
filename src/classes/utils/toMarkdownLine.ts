...
export const toMarkdownLine = (taskInstance: Task | RecurringTask): string => {
  ...
  if (taskInstance instanceof Task) {
    return `|${taskInstance.status}|${taskInstance.text}|${taskInstance.schedule || ""}|${taskInstance.start || ""}|${taskInstance.end || ""}|${taskInstance.occurrence || ""}|${taskInstance.projectLink || ""}|${tagsString}|${taskInstance.priority || ""}|${taskInstance.created || ""}|${taskInstance.completed || ""}|`;
  } else {
