...
export const openOrCreateFile = async (app: App, path: string, mode: "source" | "preview", templatePath?: string, line?: number): Promise<WorkspaceLeaf> => {
  ...
  if (existingFile) {
    workspaceLeaf = await openFile(app, existingFile as TFile, mode)
  } else {
    ...
    workspaceLeaf =  await openFile(app, newFile as TFile, mode)
  }

  if (line) {
    //@ts-ignore
    const editor = workspaceLeaf.view.editor
    const lineContent = editor.getLine(line)
    editor.setCursor({ line: line, ch: lineContent.length })
    await delay(300)
    editor.focus()
  }
